---
title: "Multi-Tenancy in Cole: Isolated School Accounts"
...
The `tenants` table stores three identifying fields:
...
The `super-admin` role is the only role whose `tenant_id` is `null`. Because the global scope and the auto-assign hook only activate when `auth()->check()` is `true` **and** the user has a non-null `tenant_id`, a super-admin can query across all tenants without restriction.
